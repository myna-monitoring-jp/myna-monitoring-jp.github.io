...
③ 朝の状況判断を生成（OpenAI Responses API + web_search）
     ・公式ページを実際に開いて今の表示を確認
     ・前日の数値との差分、確認できなかったこと、今日の優先ウォッチ
...
**機械収集の結果は材料であって成果物ではありません。** これがこの設計の中心です。

以前は収集した記事を1件1枚のカードにして全部載せていました。その結果、
概要が元記事の表題の焼き直しにしかならず、定量情報も事実関係の切り分けも
国民の声も無いカードが、人が書いた案件と同じ見た目で並んでいました。
要件の「ニュースカード必須7項目」が、手書きの項目にしか適用されていなかったのが
品質差の正体でした（検索の量ではありません）。

```
旧： 収集 → 各記事を1枚のカードにして全部載せる → 機械がタグを付ける

現： 収集（全量＝材料）
      ↓ ③が材料を全量読む（web_search で裏取り・公式ページ確認）
     載せるものを選ぶ（大半は捨てる）
      ↓
     必須項目で書き起こす
      ↓
     ポータルは書き起こしを表示。生の収集結果は「収集一覧」画面へ
```

| | ②機械収集 | ③朝の状況判断 |
|---|---|---|
| 位置づけ | **材料** | **報告** |
| 手段 | RSS・稼働状況ページ・検索 | 検索＋公式ページを実際に開いて読む |
| 見出し | 元記事の表題 | 書き直す（何が分かったかを書く） |
| 本文 | 持たない | 書く |
| 定量情報 | 持たない | 表。「未公表」も値として書く |
| 事実関係の切り分け | 持たない | 論点／判定／補足の表 |
| 「無かったこと」 | 出せない | 「新規全国オン資障害：確認なし」と明示 |
| 数値の差分 | 出せない | 「2.3/5・509件（前日466件から43件増）」 |
| 表示先 | 収集一覧 | ダッシュボード |

#### 書き起こしの構造

**ニュース**：書き直した見出し／重要度・論点タグ／A. ニュース自体＋定量テーブル／
B. 国民の声（観測されたものだけ。無ければ「確認できませんでした」）／
C. 論点・判定・補足テーブル／D. 出典（一次情報と報道を区別）

**広報**：①起点 ②訴求意図 ③現に確認できる批判 ④想定される論点 ⑤切り分け
⑥公式補足 ⑦二次拡散 ⑧広報上の注意

④は「予測される批判」であり国民の声ではありません。画面上でも別の欄にし、
「広報上の確認ポイント」と明示します（要件どおり）。

#### 収集一覧を残している理由

生の収集結果は捨てず `#/collected` に置いています。取りこぼしの検証に使えるためです。
報告に出ていない案件がこの一覧にあれば**選定**の問題、無ければ**収集**の問題と切り分けられます。

#### 設定

| 場所 | 名前 | 内容 |
|---|---|---|
| Settings → Secrets → Actions | `OPENAI_API_KEY` | 必須。`sk-` で始まる OpenAI のキー。未設定なら③をスキップして②までで公開する |
| Settings → Variables → Actions | `BRIEFING_MODEL` | 任意。既定 `gpt-6-astra` |
| 環境変数 | `MAX_BRIEFING_WRITEUPS` | 書き起こす件数の上限。既定6。増やすほど月額が上がる |

`ENABLE_BRIEFING=0` で明示的に無効化できます。ローカル実行はキーが無ければ自動で無効です。

#### HTMLを生成させない理由

**モデルにはJSONを返させ、描画はアプリのコンポーネントで行います。**
外部リンク契約（実href・`target="_blank"`・`rel="noopener noreferrer"`・空href禁止）と
状態語彙の制約は、生成されたHTMLでは検証できません。生成物の見た目が良くても、
リンクが押せない／状態ラベルが押せるように見える、といった要件違反を検出できなくなります。

#### 生成物を信用しない層

モデルの出力はスキーマを通っていても内容の約束を守りません。3段で絞っています。

1. `sanitizeBriefing()`（`scripts/collect-briefing.mjs`）
   http(s)でない出典、列挙外の差分ラベル、ラベルとしての禁止語（WATCH／継続）、件数の暴走を落とす
2. `validate-data.mjs`
   節が空、代表性の留保が欠落、出典がリンク契約違反ならエラーにして**デプロイを止める**
3. `normalizeBriefing()`（`src/data/normalize.ts`）
   ブラウザ側でもう一度絞り、節が欠けていれば「生成できませんでした」に倒す

生成に失敗した日は**前日分を流用しません**。古い判断を今朝のものとして出す方が有害だからです。
画面には「今朝の状況判断は生成できませんでした」と出て、②までの結果は通常どおり表示されます。

#### コスト

web_search は $10 / 1,000回、トークンはモデルの実費。毎朝1回で **1日 $0.5〜1.0／月 $15〜30** の想定です。
実測値は Actions のログ（`usage`）で確認できます。
...
### 4.3 「⚑ 自動収集・未レビュー」バッジの消し方

自動収集された案件には未レビューのバッジが付きます。消し方は3通りです。

| やりたいこと | 方法 | 結果 |
|---|---|---|
| **採用する** | `curated.json` に **同じ id** で項目を書き起こす | バッジが消え、あなたが書いた内容に置き換わる |
| **却下する** | `curated.json` の `dismissedIds` に id を追加 | 即座に消え、同じ記事を再収集しても復活しない |
| **放っておく** | 何もしない | **14日後に自動で消える**（その日のアーカイブJSONには残る） |

id は画面のカードの `data-item-id` 属性、または `public/data/current.json` で確認できます。

**採用するときは必ず自動項目と同じ id を使ってください。** 別の id を付けると、自動版と手書き版が二重に並びます。

却下の例：

```json
{
  "dismissedIds": ["news-auto-20260901-a1b2c3"]
}
```

放置期間は `AUTO_ITEM_RETENTION_DAYS` で変更できます（既定14日）。
...
### 4.6 `lastMaterialUpdateAt` を更新してよい場合（重要更新の定義）
