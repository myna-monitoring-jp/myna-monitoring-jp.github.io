...
public/
  ...
  sample-data/           ← サンプル。動作確認・デモ専用。本番共有には使わない
    current.json
    archive/2026-08-31.json
    archive/2026-08-26.json
```

両者は **ディレクトリごと分離** されており、`VITE_DATA_SOURCE` でしか切り替わりません。サンプル表示中は常に画面上部に紫のバナーが出て、Teams投稿文にも「サンプルデータでの出力です」が入るため、取り違えて共有できないようになっています。
...
### 4.3 `lastMaterialUpdateAt` を更新してよい場合（重要更新の定義）
