...
②までは「何が公開されたか」しか取れません。RSSとキーワード一致では、
**公式ページが今どう表示しているか／前日の数値と比べてどう動いたか／確認できなかったこと**
が取れず、ここが最大の抜けでした。③がその層を担います。

| | ②機械収集 | ③朝の状況判断 |
|---|---|---|
| 手段 | RSS・稼働状況ページ・検索 | 検索＋公式ページを実際に開いて読む |
| 出すもの | 記事・障害の一覧 | 総括・判断・前日差分・優先ウォッチ・留保 |
| 「無かったこと」 | 出せない | 「新規全国オン資障害：確認なし」と明示 |
| 数値 | 出せない | 「2.3/5・509件（前日466件から43件増）」 |

#### 設定

| 場所 | 名前 | 内容 |
|---|---|---|
| Settings → Secrets → Actions | `OPENAI_API_KEY` | 必須。未設定なら③をスキップして②までで公開する |
